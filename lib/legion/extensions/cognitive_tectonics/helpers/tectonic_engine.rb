      ...
      module Helpers
        class TectonicEngine
          ...
          def detect_collisions
            ...
            pairs  = active.combination(2).select do |a, b|
              a.distance_to(b) < Constants::COLLISION_THRESHOLD
            end
          ...
          def trigger_earthquake(plate_id:, magnitude:, **)
            ...
            event  = SeismicEvent.new(
              type:               :earthquake,
              magnitude:          magnitude,
              epicenter_plate_id: plate_id,
              affected_plate_ids: nearby.map(&:id)
            )
          ...
          def tectonic_report
            active_plates    = @plates.values.select(&:active?)
            subducted_plates = @plates.values.reject(&:active?)
